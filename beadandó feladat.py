...
class EgyagyasSzoba(Szoba):
    def __init__(self, szobaszam, ar=5000):
...
class KetagyasSzoba(Szoba):
    def __init__(self, szobaszam, ar=8000):
        super().__init__(ar, szobaszam)
...
class Szalloda:
    ...
    def f_hozzadas(self, szobaszam, kezdet, vege):
        if vege <= kezdet:
            return None, "A befejező dátum nem lehet korábbi, mint a kezdő dátum."
        ...
        if kezdet < datetime.now():
            return None, "A kezdő dátum nem lehet a múltban."
...
def cli(szalloda):
    while True:
        ...
        if valasztas == "1":
            ...
            kezdet = datetime.strptime(input("Adja meg a kezdő dátumot (YYYY-MM-DD): "), "%Y-%m-%d")
            vege = datetime.strptime(input("Adja meg a befejező dátumot (YYYY-MM-DD): "), "%Y-%m-%d")
